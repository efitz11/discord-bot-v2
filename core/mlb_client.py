...
class MLBClient:
    ...
    async def _fill_bench_handedness(self, box) -> None:
        """Populate bench and batting rows' batting hand (L/R/S) via one batch lookup —
        the boxscore payload doesn't include batSide."""
        row_groups = [g for g in (getattr(box, "bench_rows", None), getattr(box, "batting_rows", None)) if g]
        if not row_groups:
            return
        ids = {str(r["id"]) for group in row_groups for r in group if r.get("id")}
        if not ids:
            return
        session = await self.get_session()
        try:
